...
| `static/` | UI — `index.html`, `style.css`, `app.js`. No build step, no framework. `manifest.webmanifest` (+ `<link rel="manifest">`) makes the Edge/Chrome `--app` window adopt Hangar's icon for the window/taskbar instead of Edge's default; `app.py` registers the `.webmanifest` mimetype so it's served as `application/manifest+json`. |
...
- **Open in Blender:** `/api/assets/<id>/open-blender` *launches* Blender (`thumbs.find_blender()`) — a `.blend` opens directly; other models import via `--python-expr` (`_IMPORT_OPS`, newest-API-first with fallback). Different from send-blender, which needs the add-on already running. Surfaced in the tile right-click menu for `kind == "model"` (`openAssetInBlender`).
- **Render-farm worker download:** `GET /api/farm/worker-download` zips the standalone worker (`worker_main.py`+`worker.py`+`store.py`+`thumbs.py`+`scanner.py` from `BASE_DIR`, plus generated `requirements.txt`/`run.bat`/`run.sh`/`README.txt` pre-filled with this Hangar's `request.host_url` + `FARM_TOKEN`). Button `#farmDownload` in the farm panel. **Frozen builds must ship those sources** — both CI workflows add them via `--add-data` (else the endpoint logs "missing source files" and the zip is script-only).
