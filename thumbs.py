...
def verify_blend(path):
    ...
    try:
        ...
        if head[8:9] != b"v":
            # Not the classic little-endian layout this walker understands
            # (big-endian relics, or a future block format). Don't flag what we
            # can't actually verify — a false "damaged" would invite a bogus
            # repair of a healthy file.
            return True, ""
        is_64 = head[7:8] == b"-"
        endian = "<"
        bhead_size = 24 if is_64 else 20
...
def _blend_header(path):
    """First 12 bytes of a .blend's (decompressed) stream — enough to know the
    Blender version, pointer size and endianness. Cheap even for compressed
    files: only the start of the first frame gets decoded."""
    try:
        with open(_fs(path), "rb") as fh:
            head = fh.read(12)
        if head[:2] == b"\x1f\x8b":
        ...
        if head[:4] == b"\x28\xb5\x2f\xfd":
            ...
            with open(_fs(path), "rb") as fh:
                return zstandard.ZstdDecompressor().stream_reader(
                    fh, read_across_frames=True).read(12)
        return head
    except Exception:
        return None


def _generate_dna_donor(want_head):
    """No library file matches the damaged file's Blender version? Make one:
    the installed Blender saves an empty scratch .blend, which carries its
    build's DNA catalog by construction. That covers the most common damage
    here — Hangar's own interrupted saves — because the Blender that wrote the
    truncated file is the very one Hangar still uses. Cached in the data dir
    and regenerated only when the wanted header changes."""
    blender = find_blender()
    if not blender:
        return None
    out = str(THUMB_DIR.parent / "dna-donor.blend")
    if os.path.exists(_fs(out)) and _blend_header(out) == want_head:
        return out
    import subprocess
    expr = (f"import bpy\n"
            f"bpy.ops.wm.save_as_mainfile(filepath=r'''{out}''', compress=False)\n")
    try:
        subprocess.run(
            [blender, "--background", "--factory-startup", "--disable-autoexec",
             "--python-expr", expr],
            timeout=180, capture_output=True, env=_blender_env(), **_no_window(),
        )
    except Exception:
        return None
    if os.path.exists(_fs(out)) and _blend_header(out) == want_head:
...
def repair_blend(path, donor_paths=()):
    ...
    head = raw[:12]
    if head[8:9] != b"v":
        return {"ok": False, "error": "Unrecognised .blend block layout — "
                                      "this walker can't rebuild it."}
    is_64 = head[7:8] == b"-"
    endian = "<"
    bh = 24 if is_64 else 20
    pos = 12
    ...
    if saw_dna:
        ...
        dna = None
        # Header peek first (12 bytes) — only version-matching donors get read
        # in full, so a big candidate list costs almost nothing.
        matching = (dp for dp in donor_paths if _blend_header(dp) == head)
        for tried, dp in enumerate(matching):
            if tried >= 20:
                break
            try:
            ...
            if dna:
                ...
                break
        if not dna:
            # Last resort: have the installed Blender save a scratch file — a
            # guaranteed-correct donor whenever it's the same version that
            # wrote the damaged file.
            gen = _generate_dna_donor(head)
            if gen:
                dna = _extract_dna1(gen, head)
                donor_used = "generated by your installed Blender"
        if not dna:
            raw_ver = head[9:12].decode("ascii", "replace")
            try:
                ver = f"{raw_ver[0]}.{int(raw_ver[1:])}"   # "306" -> "3.6", "279" -> "2.79"
            except ValueError:
                ver = raw_ver
            return {"ok": False, "error":
                    f"No DNA catalog for Blender {ver} available — no healthy "
                    f"library file matches that version and your installed "
                    f"Blender is a different one. Install Blender {ver} (or add "
                    f"any healthy .blend saved by it to a library) and retry."}
